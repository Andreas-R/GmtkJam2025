...
func spawn():
    ...
    spawn_tween.tween_property(asteroid, "position", Vector2(-asteroid.position.x, asteroid.position.y), 4.0).set_delay(4.0)
    spawn_tween.tween_property(indicator, "scale", Vector2(indicator.scale.x, 0), 0.5).set_delay(7.0)
    spawn_tween.tween_callback(cleanup).set_delay(8.0)
